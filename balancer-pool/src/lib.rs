...
use btoken::{BToken, FungibleToken, ext_nep21};
...
//            total_supply: 0,
//            accounts: UnorderedMap::new("a".to_vec()),
    ...
    pub fn isBound(&self, token: AccountId) -> bool {
        self.records.get(&token).map(|record| record.bound).unwrap_or(false)
    }
    ...
    pub fn setSwapFee(&mut self, swapFee: U128) {
        ...
        assert_eq!(env::predecessor_account_id(), self.controller, "ERR_NOT_CONTROLLER");
    ...
    pub fn rebind(&mut self, token: AccountId, balance: Balance, denorm: Weight) {
        ...
        if balance > old_balance {
            self.pull_underlying(&token, &env::predecessor_account_id(), balance - old_balance);
        } else {
            let token_balance_withdrawn = old_balance - balance;
            let token_exit_fee = token_balance_withdrawn * EXIT_FEE;
            self.push_underlying(&token, &env::predecessor_account_id(), token_balance_withdrawn - token_exit_fee);
            self.push_underlying(&token, &self.factory.clone(), token_exit_fee);
        }
    ...
    pub fn getSpotPrice(&self, tokenIn: AccountId, tokenOut: AccountId) -> Balance {
        ...
        calc_spot_price(in_record.balance, in_record.denorm, out_record.balance, out_record.denorm, self.swap_fee)
    }
    ...
    pub fn getSpotPriceSansFee(&self, tokenIn: AccountId, tokenOut: AccountId) -> Balance {
        ...
        let in_record = self.records.get(&tokenIn).unwrap();
        let out_record = self.records.get(&tokenOut).unwrap();
        calc_spot_price(in_record.balance, in_record.denorm, out_record.balance, out_record.denorm, 0)
    }
    ...
    pub fn joinPool(&mut self, poolAmountOut: Balance, maxAmountsIn: Vec<Balance>) {
        ...
        for i in 0..self.tokens.len() {
            ...
            self.pull_underlying(&self.tokens[i].clone(), &env::predecessor_account_id(), token_amount_in);
    ...
    pub fn exitPool(&mut self, poolAmountIn: Balance, minAmountsOut: Vec<Balance>) {
        ...
        for i in 0..self.tokens.len() {
            ...
            self.push_underlying(&self.tokens[i].clone(), &env::predecessor_account_id(), token_amount_out);
        }
    }
...
impl BPool {
    fn pull_underlying(&mut self, token: &AccountId, from: &AccountId, amount: Balance) -> Promise {
        ext_nep21::transfer_from(from.clone(), env::current_account_id(), amount.into(), token, NO_DEPOSIT, gas::NEP21_TRANSFER_FROM)
            .then(ext_self::on_pull(&env::current_account_id(), NO_DEPOSIT, gas::ON_PULL_CALLBACK))
    }

    fn push_underlying(&mut self, token: &AccountId, to: &AccountId, amount: Balance) -> Promise {
        ext_nep21::transfer(to.clone(), amount.into(), token, NO_DEPOSIT, gas::NEP21_TRANSFER)
            .then(ext_self::on_push(&env::current_account_id(), NO_DEPOSIT, gas::ON_PUSH_CALLBACK))
    }
...
mod tests {
    use super::*;
    use near_sdk::{VMContext, testing_env, MockedBlockchain};

    fn pool_account() -> AccountId {
        "pool".to_string()
    }
    fn factory_account() -> AccountId { "factory".to_string() }
    fn token1_account() -> AccountId { "token1".to_string() }
    fn token2_account() -> AccountId { "token2".to_string() }
    ...
    fn test_setup_pool() {
        ...
        pool.bind(token1_account(), to_yocto(50_000).into(), to_yocto(10).into());
        pool.bind(token2_account(), to_yocto(1_000_000).into(), to_yocto(10).into());
        pool.finalize();
